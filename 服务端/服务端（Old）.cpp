...
void handleLogin(SOCKET sock, const string& loginMessage) {
    string username = loginMessage.substr(0, loginMessage.find(' '));
    bool adminFlag = true;
    isAdmin[username] = adminFlag;

    User newUser;
    newUser.username = username;
    newUser.isAdmin = adminFlag;
    userDatabase[username] = newUser;

    string response = username + " is now logged in. Admin status: " + (adminFlag ? "True" : "False");
    sendToClient(sock, response);
}
