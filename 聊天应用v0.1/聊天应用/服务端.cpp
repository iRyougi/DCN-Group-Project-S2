...
map <string, User> userDatabase;  // 用来存储用户信息的数据库
...
int main()
{
	...
	connect;
		...
		while (1)
		{
			...
			sprintf(sendBuf, "[DCN Ass 2]%s", contentBuf);
		...
		for (int i = 0; i < total + 1; i++)//i代表现在正在监听事件的终端
		{
			...
			else if (index == 0)
			{
				...
				if (networkEvents.lNetworkEvents & FD_ACCEPT)//若产生accept事件（此处与位掩码相与）
				{
					...
					if (total + 1 < MAX_LINK_NUM)//若增加一个客户端仍然小于最大连接数，则接受该链接
					{
						...
						if (newSock != INVALID_SOCKET)
						{
							...
							//给所有客户端发送欢迎消息
							char buf[BUFFER_SIZE] = "[DMS Ass 2]欢迎（IP：";
				...
				else if (networkEvents.lNetworkEvents & FD_CLOSE)//客户端被关闭，即断开连接
				{
					...
					//i表示已关闭的客户端下标
					total--;
					cout << "#" << i << "（IP：" << inet_ntoa(cliAddr[i].sin_addr) << ")退出了聊天室,当前连接数：" << total << endl;
					//释放这个客户端的资源
					...
					//给所有客户端发送退出聊天室的消息
					char buf[BUFFER_SIZE] = "[DMS Ass 2]（IP：";
				...
				else if (networkEvents.lNetworkEvents & FD_READ) //接收到消息
				{
					...
					int nrecv = recv(cliSock[i], buffer, sizeof(buffer), 0); //接收数据
					if (nrecv > 0) //如果接收到的字符数大于0
					{
						// 检查是否为注册请求
						if (strncmp(buffer, "REG:", 4) == 0) {
							// 解析用户名
							string username = strtok(buffer + 4, "|");
							// 尝试注册用户
							if (registerUser(username)) {
								send(cliSock[i], "Registration successful", 30, 0);
							}
							else {
								send(cliSock[i], "Username already taken", 23, 0);
							}
						}
